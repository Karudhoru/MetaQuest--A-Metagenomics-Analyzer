...
def run_analysis(input_file, file_type, output_dir):
    """Main analysis controller that calls the correct pipeline and then generates the dashboard."""
    output_dir_path = Path(output_dir)

    if file_type == 'fastq':
        analyze_fastq(input_file, output_dir_path)
    else:
        ...
        analyze_fasta(Path(fasta_path), output_dir_path)
